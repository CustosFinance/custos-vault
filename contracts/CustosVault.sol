...
import {IRageQuit} from "../hypercustos/HypercustosUniV3.sol";
    ...
    IERC721Receiver
{
    ...
    bytes32 public constant LOCK_TYPEHASH =
        keccak256(
            "LockERC20(address delegate,address token,uint256 amount,uint256 nonce)"
        );
    bytes32 public constant UNLOCK_TYPEHASH =
        keccak256(
            "UnlockERC20(address delegate,address token,uint256 amount,uint256 nonce)"
        );
    bytes32 public constant LOCK_ERC721_TYPEHASH =
        keccak256(
            "LockERC721(address delegate,address token,uint256 tokenId,uint256 nonce)"
        );
    bytes32 public constant UNLOCK_ERC721_TYPEHASH =
        keccak256(
            "UnlockERC721(address delegate,address token,uint256 tokenId,uint256 nonce)"
        );
    ...
    function getPermissionHash(
        ...
        return
            EIP712._hashTypedDataV4(
                keccak256(
                    abi.encode(eip712TypeHash, delegate, token, amount, nonce)
                )
            );
    }
    ...
    function getPermissionHashERC721(
        ...
        return
            EIP712._hashTypedDataV4(
                keccak256(
                    abi.encode(eip712TypeHash, delegate, token, tokenId, nonce)
                )
            );
    }
        ...
        returns (bool validity)
    {
        ...
        for (uint256 index; index < count; index++) {
            ...
            if (
                IERC20(_lockData.token).balanceOf(address(this)) <
                _lockData.balance
            ) return false;
        }
        ...
        returns (bool validity)
    {
        ...
        for (uint256 index; index < count; index++) {
            ...
            // if insufficient balance return false
            if (
                IERC721(_lockData.token).balanceOf(address(this)) <
                _lockData.balance
            ) return false;
        }
        // if sufficient balance return true
